...
import { getRecordIndex } from "./PageInformation/PageInfo";
import { searchInUsers } from "./PageInformation/SearchInfo";
...
function App() {
  ...
  const selectAll = (e) => {
    const listedUserIds = users
      ...
      .slice(index, index + process.env.REACT_APP_PAGE_SIZE)
      .map((user) => user.id);
